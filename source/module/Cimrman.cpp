...
namespace cimrman {
  ...
  {
    Cimrman & cimrman; ///< Reference to the father recontruction module
    ...
    // Constructor
    pimpl_type(Cimrman & cimrman_);
  };

  Cimrman::pimpl_type::pimpl_type(Cimrman & cimrman_)
    : cimrman(cimrman_)
  {
    ...
    palgo->initialize(cimrman._config_.recConfig);
  ...
  void Cimrman::_populate_working_event_(const datatools::things &workItem)
  {
    ...
    if(workItem.has(_config_.CD_label))
    {
	    ...
	    for (const auto & trhit : falaiseCDbank.tracker_hits() )
      {
        ...
        auto hit = std::make_shared<datamodel::TrackerHit>(SRL);
       ...
       && workItem.has(_config_.TCD_label))
    {
      ...
      for(const auto & cluster : solution->get_clusters())
      {
        ...
        std::vector<datamodel::TrackerHitHdl> temp_precluster;
      ...
      {   
        ...
        for(auto & trajectory : trajectories)
        {
          ...
          for(auto & segment : traj_segments)
          {
            ...
            {
              if(auto delayed_cluster = std::dynamic_pointer_cast<const cimrman::datamodel::DelayedCluster>(cluster_ptr))
              {
